...
__device__ uint32_t  Te0_ConstMem[256] = {
...
__device__ uint32_t  Te1_ConstMem[256] = {
...
__device__ uint32_t  Te2_ConstMem[256] = {
...
__device__ uint32_t  Te3_ConstMem[256] = {
...
static __host__ __device__ uint32_t GETU32(const unsigned char *pt)
{
...
static  __device__ void PUTU32(uint8_t *ct, uint32_t st)
{
	*((uint32_t*)ct) = ((st >> 24) ^
		(((st << 8) >> 24) << 8) ^
		(((st << 16) >> 24) << 16) ^
...
#define SHARED_MEM_BANK_SIZE		32 	// 32 avoids bank conflict
#define TABLE_SIZE					256
#define ROUND						13 	//AES-256
...
static __device__ uint32_t BytePerm(uint32_t x, uint32_t n) { return __byte_perm(x, x, n); }

static __device__ void encAES256OneTblReuseRpt(uint8_t *out, const uint32_t *rk, uint32_t counter, uint32_t nblocks){

	uint32_t t0, t1, t2, t3;
	int warpThreadIndex = threadIdx.x %SHARED_MEM_BANK_SIZE;	

	__shared__ uint32_t t0S[TABLE_SIZE][SHARED_MEM_BANK_SIZE];
	__shared__ uint32_t rkS[AES_256_KEY_SIZE_INT];
	...
	if (threadIdx.x < TABLE_SIZE) {
		...
		for (uint32_t i = 0; i < SHARED_MEM_BANK_SIZE; i++) {
			t0S[wid + i* (TABLE_SIZE/SHARED_MEM_BANK_SIZE)][threadIdx.x%SHARED_MEM_BANK_SIZE] = Te0_ConstMem[wid + i*(TABLE_SIZE/SHARED_MEM_BANK_SIZE)];
		}
	...
	s1 = 0 ^ rkS[1];
	s2 = 0 ^ rkS[2];
	s3 = counter ^ rkS[3];	// Only use 32-bit counter
	
	// Generates 8*16 bytes encryption for each nblock
	for(uint8_t m = 0; m < 8*nblocks; m++) {
		...
		// Last round uses s-box directly and XORs to produce output. The s-box is actually from t0S, just shift the positions.
		s0 = ((t0S[t0 >> 24][warpThreadIndex] & 0x0000ff00) <<16)^ ((t0S[t1 >> 16& 0xff][warpThreadIndex] & 0x0000ff00) <<8) ^ ((t0S[t2 >> 8 & 0xff][warpThreadIndex] & 0x0000ff00)) ^ ((t0S[t3 & 0xff][warpThreadIndex] & 0x0000ff00 )>>8) ^ rkS[56];
		s1 = ((t0S[t1 >> 24][warpThreadIndex] & 0x0000ff00) <<16) ^ ((t0S[t2 >> 16& 0xff][warpThreadIndex] & 0x0000ff00) <<8) ^ ((t0S[t3 >> 8 & 0xff][warpThreadIndex] & 0x0000ff00)) ^ ((t0S[t0 & 0xff][warpThreadIndex] & 0x0000ff00) >>8) ^ rkS[57];
		s2 = ((t0S[t2 >> 24][warpThreadIndex] & 0x0000ff00) <<16) ^ ((t0S[t3 >> 16& 0xff][warpThreadIndex] & 0x0000ff00) <<8) ^ ((t0S[t0 >> 8 & 0xff][warpThreadIndex] & 0x0000ff00)) ^ ((t0S[t1 & 0xff][warpThreadIndex] & 0x0000ff00) >>8) ^ rkS[58];
		s3 = ((t0S[t3 >> 24][warpThreadIndex] & 0x0000ff00) <<16) ^ ((t0S[t0 >> 16& 0xff][warpThreadIndex] & 0x0000ff00) <<8) ^ ((t0S[t1 >> 8 & 0xff][warpThreadIndex] & 0x0000ff00)) ^ ((t0S[t2 & 0xff][warpThreadIndex] & 0x0000ff00)>>8) ^ rkS[59];

		// The global memory store is not coalesced, can we solve this?
		PUTU32(out + m*16, s0);
		PUTU32(out + m*16 + 4, s1);
		PUTU32(out + m*16 + 8, s2);
		PUTU32(out + m*16 + 12, s3);
	}
}
// // Comment, proposed One-T and reuse T0 for SBox (Fastest)
...
static __device__ void encAES256OneTblReuse(uint8_t *out, const uint32_t *rk, uint32_t counter){
	...
	// Each warp loads 32 values, total TABLE_SIZE/SHARED_MEM_BANK_SIZE	banks are involved.
	if (threadIdx.x < TABLE_SIZE) {
		// t0S[threadIdx.x][0] = t0G[threadIdx.x];		
		// __syncthreads();
		uint32_t wid = threadIdx.x / SHARED_MEM_BANK_SIZE;
		for (uint32_t i = 0; i < SHARED_MEM_BANK_SIZE; i++) {
			t0S[wid + i* (TABLE_SIZE/SHARED_MEM_BANK_SIZE)][threadIdx.x%SHARED_MEM_BANK_SIZE] = Te0_ConstMem[wid + i*(TABLE_SIZE/SHARED_MEM_BANK_SIZE)];
	...
	// The s-box is actually from t0S, just shift the positions.
	s0 = ((t0S[t0 >> 24][warpThreadIndex] & 0x0000ff00) <<16)^ ((t0S[t1 >> 16& 0xff][warpThreadIndex] & 0x0000ff00) <<8) ^ ((t0S[t2 >> 8 & 0xff][warpThreadIndex] & 0x0000ff00)) ^ ((t0S[t3 & 0xff][warpThreadIndex] & 0x0000ff00 )>>8) ^ rkS[56];
	s1 = ((t0S[t1 >> 24][warpThreadIndex] & 0x0000ff00) <<16) ^ ((t0S[t2 >> 16& 0xff][warpThreadIndex] & 0x0000ff00) <<8) ^ ((t0S[t3 >> 8 & 0xff][warpThreadIndex] & 0x0000ff00)) ^ ((t0S[t0 & 0xff][warpThreadIndex] & 0x0000ff00) >>8) ^ rkS[57];
	s2 = ((t0S[t2 >> 24][warpThreadIndex] & 0x0000ff00) <<16) ^ ((t0S[t3 >> 16& 0xff][warpThreadIndex] & 0x0000ff00) <<8) ^ ((t0S[t0 >> 8 & 0xff][warpThreadIndex] & 0x0000ff00)) ^ ((t0S[t1 & 0xff][warpThreadIndex] & 0x0000ff00) >>8) ^ rkS[58];
	s3 = ((t0S[t3 >> 24][warpThreadIndex] & 0x0000ff00) <<16) ^ ((t0S[t0 >> 16& 0xff][warpThreadIndex] & 0x0000ff00) <<8) ^ ((t0S[t1 >> 8 & 0xff][warpThreadIndex] & 0x0000ff00)) ^ ((t0S[t2 & 0xff][warpThreadIndex] & 0x0000ff00)>>8) ^ rkS[59];
...
static __device__ void encAES256(uint8_t *out, const uint32_t *roundkey, uint32_t counter)
{
	...
	/* initialize T boxes, #threads in block should be larger than 256. Thread 0 - 255 cooperate to copy the T-boxes from constant mem to shared mem*/
	if(threadIdx.x < 256)
	{
...
// 1 AES ==> nblocks * 8 CTR (3 * 8 * 16 = 384 bytes)
// 1 thread ==> repeat 4 to 5 times (rep), assume it is 5 times
// 1 block ==> 5 * THREAD * 24
static  __device__ void aes256ctr_squeezeblocks_gpu(uint8_t *out,
                           unsigned long long nblocks, const uint32_t *rk, uint32_t rep)
{  	
  	uint32_t tid = threadIdx.x, bid = blockIdx.x;

  // 	for(j=0;j<nblocks;j++) {
  //   	// aesni_encrypt8(out, &state->n, state->rkeys);    
  //   	for(i=0; i<8; i++) encAES256(out + j*128 + i*16, rk, bid*5*THREAD*24 +  rep*THREAD*24 + tid*24 + j*8 + i);
		// // for(i=0; i<8; i++) encAES256OneTblBytePermReuse(out + j*128 + i*16, rk, bid*5*THREAD*24 +  rep*THREAD*24 + tid*24 + j*8 + i);
		// // out += 128;        		
  // 	}
	encAES256OneTblReuseRpt(out, rk, bid*5*THREAD*24 + rep*THREAD*24 + tid*24, nblocks);		
}

static void AESPrepareKey(char *dec_key, const unsigned char *enc_key, unsigned int key_bits)
{
