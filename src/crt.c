...
{
	uint64_t i,j,k, bitsize1, bitsize2;
	...
	for (k = 0; k < SIFE_N; k++) {
		...
		for (i = 1; i <= SIFE_NMODULI-1; i++) {
			...
			// bitsize1 = mpz_sizeinbase(gmp_u, 2);
			// bitsize2 = mpz_sizeinbase(gmp_x, 2);
			// printf("i: %lu bitsize: %lu %lu\n", i, bitsize1, bitsize2);	
		}
		mpz_set(a[k],gmp_x);
	}
...
void
crt_reverse(mpz_t a[SIFE_N], const uint32_t split_a[SIFE_NMODULI][SIFE_N])
{
	uint64_t i,j,k, bitsize1, bitsize2;
	// mpz_t gmp_u, gmp_c, gmp_x;
